...
public class AStar {
	...
	public static List<Node> findPath(Mundo world, Vector2i start, Vector2i end){
		...
		while(openList.size() > 0) {
			...
			for(int i = 0; i < 9; i++) {
				...
				int xi = (i%3) - 1;
				int yi = (i/3) - 1;
				if (!(x+xi<0 || y+yi<0 || x+xi >= Mundo.WIDTH || y+yi >= Mundo.HEIGHT)) {
					Tile tile = Mundo.tiles[x+xi+((y+yi)*Mundo.WIDTH)];
					if(tile == null) continue;
					if(tile instanceof WallTile) continue;
					if(i == 0) {
						Tile test = Mundo.tiles[x+xi+1+((y+yi) * Mundo.WIDTH)];
						Tile test2 = Mundo.tiles[x+xi+((y+yi+1) * Mundo.WIDTH)];
						...
						Tile test = Mundo.tiles[x+xi+((y+yi-1) * Mundo.WIDTH)];
						Tile test2 = Mundo.tiles[x+xi+1+((y+yi) * Mundo.WIDTH)];
							if(test instanceof WallTile || test2 instanceof WallTile) {
								continue;
							}
					}
