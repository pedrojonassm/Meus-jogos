		...
		try {
			...
			for(int xx = 0; xx < map.getWidth(); xx++){
				for(int yy = 0; yy < map.getHeight(); yy++){
					...
					}else if (pixelAtual == 0xFF938E00) {
						// criador de inimigos
						tiles[xx + (yy * WIDTH)] = new FloorTile(xx*16,yy*16,Tile.TILE_FLOOR);
						Criador criador = new Criador(xx*16, yy*16, 16, 16, 0, null);
						Game.entities.add(criador);
					}else if (pixelAtual == 0xFF00FFA9) {
						// chegada dos inimigos
						tiles[xx + (yy * WIDTH)] = new FloorTile(xx*16,yy*16,Tile.TILE_FLOOR);
						Player chegada = new Player(xx*16, yy*16, 16, 16, 0, null);
						Game.chegada = chegada;
						Game.entities.add(chegada);
					}
				}
			}
		} catch (IOException e) {
