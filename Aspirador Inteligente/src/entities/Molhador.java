...
public class Molhador extends Entity {
	
	private int Ix, Iy;
	
	public Molhador(double x, double y, int width, int height, double speed, BufferedImage sprite) {
	...
	public void tick() {
		if (explorar) {
			...
			if (Mundo.tiles[px+py*Mundo.WIDTH].estado == 2) {
				...
				Network.MoExplorados.add(Mundo.tiles[px+py*Mundo.WIDTH]);
				Network.MoConhecidos.remove(Mundo.tiles[px+py*Mundo.WIDTH]);
				if (Game.network.verifica3(Mundo.tiles[px+py*Mundo.WIDTH])) {
					Network.SeConhecidos.add(Mundo.tiles[px+py*Mundo.WIDTH]);
					for (int i = 0; i < Game.network.agentes.size(); i++) {
						if (Game.network.agentes.get(i) instanceof Secador) {
							Game.network.agentes.get(i).explorar = true;
						}
					}
				}
			}
			if((path == null || path.size() == 0) && Network.MoConhecidos.size() > 0) {
			...
			if (Network.MoConhecidos.size() == 0) {
				explorar = false;
				Vector2i start = new Vector2i( (int)(this.getX()/16),(int)this.getY()/16);
	...
	private Tile maisProximo() {
		double d = -1, d1;
		int k = 0;
		for (int i = 0; i < Network.MoConhecidos.size(); i++) {
			Tile z = Network.MoConhecidos.get(i);
			boolean pode = true;
		...
		return Network.MoConhecidos.get(k);
	}
