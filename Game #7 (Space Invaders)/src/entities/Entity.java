...
public class Entity {
	...
	public boolean debug = false, escolhido = false, controlado = false, right = false, left = false, up = false, down = false;
	...
	public void dentroParede() {
		int tileX = getX()/16, tileY = getY()/16;
		boolean dentro = true;
		
		for (int d = 0; dentro; d++) {
			for (int ty = d*-1; ty <= d && dentro; ty++) {
				if (tileY + ty < Mundo.HEIGHT && tileY+ty > 0) {
					for (int tx = d*-1; tx<=d && dentro; tx++) {
						if (tileX + tx < Mundo.WIDTH && tileX+tx > 0) {
							if ((ty == d*-1 || tx == d*-1 || ty == d || tx == d) && dentro) {
								if (Mundo.tiles[(tileX+tx)+(tileY+ty)*Mundo.WIDTH] instanceof FloorTile) {
									dentro = false;
									setX((tileX+tx)*Game.TS);
									setY((tileY+ty)*Game.TS);
								}
							}
						}
					}
